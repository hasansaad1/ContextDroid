...
class NavGraphBfsExploreStrategy:
    """Default strategy — current nav-graph BFS tier walk."""

    def pick_action(self, turn: ExploreTurnInput, state: ExploreState) -> ExplorePickResult:
        return choose_explore_action(turn, state)


class NoOpExploreStrategy:
...
DEFAULT_EXPLORE_STRATEGY: ExploreStrategy = NavGraphBfsExploreStrategy()


def resolve_explore_strategy() -> ExploreStrategy:
    import os

    name = str(os.environ.get("CONTEXTDROID_EXPLORE_STRATEGY") or "").strip().lower()
    if name == "noop":
        return NoOpExploreStrategy()
    return NavGraphBfsExploreStrategy()
