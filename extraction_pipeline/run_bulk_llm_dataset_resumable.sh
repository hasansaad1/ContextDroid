#!/usr/bin/env bash
# Resumable bulk LLM dataset collection: one APK at a time, fixed duration per app.
#
# Usage (repo root):
#   bash extraction_pipeline/run_bulk_llm_dataset_resumable.sh [apk_root] [duration_sec]
#
# Defaults: data/apks/benign, 1200s (20 min) per app.
#
...
# Build / refresh APK manifest (sorted, stable order).
"${PYTHON_BIN}" - "${APK_ROOT}" "${MANIFEST_FILE}" <<'PY'
import sys
...
out.write_text("\n".join(str(p.resolve()) for p in apks) + ("\n" if apks else ""), encoding="utf-8")
print(len(apks))
PY
TOTAL_APKS="$("${PYTHON_BIN}" - "${MANIFEST_FILE}" <<'PY'
from pathlib import Path
import sys
print(sum(1 for ln in Path(sys.argv[1]).read_text().splitlines() if ln.strip()))
PY
)"

log "Bulk LLM dataset: ${TOTAL_APKS} APKs under ${APK_ROOT}, ${DURATION}s each"
log "State: ${STATE_DIR}"
log "Index: ${INDEX_CSV}"
