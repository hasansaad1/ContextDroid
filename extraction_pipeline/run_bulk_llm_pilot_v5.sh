#!/usr/bin/env bash
# Pilot v5: 25-app bulk re-run (same APK set + order as pilot v4) with Phase 1–3 agent
# improvements: digest-driven goals, BFS layer deepening + hub scoping, foreground recovery,
# back-goal guards, sparse primary-UX blend. Compare against logs/bulk_llm_pilot_v4/.
#
# Usage (repo root):
#   bash extraction_pipeline/run_bulk_llm_pilot_v5.sh [duration_sec]
#
# Logs: logs/bulk_llm_pilot_v5/
# APK set: data/apks/pilot_v2/ — identical 25 symlinks used by pilot v3/v4.
...
LOG_DIR="${BASE_DIR}/logs/bulk_llm_pilot_v5"
...
V4_STATE="${BASE_DIR}/logs/bulk_llm_pilot_v4/state"
if [[ -f "${V4_STATE}/pilot_manifest.txt" ]]; then
  cp "${V4_STATE}/pilot_manifest.txt" "${LOG_DIR}/state/pilot_manifest.txt"
elif [[ -f "${BASE_DIR}/logs/bulk_llm_pilot_v3/state/pilot_manifest.txt" ]]; then
  cp "${BASE_DIR}/logs/bulk_llm_pilot_v3/state/pilot_manifest.txt" \
