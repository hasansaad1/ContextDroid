#!/usr/bin/env bash
# Pilot v3: 25-app bulk re-run with timing fixes + periodic emulator reboot.
#
# Usage (repo root):
#   bash extraction_pipeline/run_bulk_llm_pilot_v3.sh [duration_sec]
#
# Logs: logs/bulk_llm_pilot_v3/
# APK set: data/apks/pilot_v2/ (same 25 apps as pilot v2)
...
LOG_DIR="${BASE_DIR}/logs/bulk_llm_pilot_v3"
...
if [[ -f "${BASE_DIR}/logs/bulk_llm_pilot_v2/state/pilot_manifest.txt" ]]; then
