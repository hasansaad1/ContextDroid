#!/usr/bin/env bash
# Phase 1 goal-synthesis spot-check: mobilizon + protonvpn (same env as pilot v4).
set -euo pipefail

BASE_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
PYTHON_BIN="${PYTHON_BIN:-${BASE_DIR}/.venv/bin/python}"
ADB_BIN="${ADB_BIN:-${BASE_DIR}/tools/platform-tools/adb}"
APK_ROOT="${BASE_DIR}/data/apks/pilot_v2"
LOG_ROOT="${LOG_ROOT:-${BASE_DIR}/logs/phase1_goal_spotcheck}"
...
APPS=(
  ...
  "ch.protonvpn.android:ch.protonvpn.android_605177200.apk"
)
...
log "Spot-check complete: ${LOG_ROOT}"
